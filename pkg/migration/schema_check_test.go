...
func TestValidateSchemaPlacement(t *testing.T) {
	...
	t.Run("data in active schema with leftovers elsewhere", func(t *testing.T) {
		require.NoError(t, validateSchemaPlacement("norna", "norna", []string{"public", "norna"}))
	})
	t.Run("data only in another schema", func(t *testing.T) {
		err := validateSchemaPlacement("public", "public", []string{"norna"})
		require.Error(t, err)
		assert.Contains(t, err.Error(), "norna")
	...
	t.Run("configured schema does not exist", func(t *testing.T) {
		err := validateSchemaPlacement("norna", "public", nil)
		require.Error(t, err)
		assert.Contains(t, err.Error(), "does not exist")
		assert.Contains(t, err.Error(), `"norna"`)
	})
	t.Run("no valid schema in search_path", func(t *testing.T) {
		err := validateSchemaPlacement("", "", []string{"norna"})
		require.Error(t, err)
		assert.Contains(t, err.Error(), "does not exist")
	})
}
