...
describe('parseTaskIdFromUrl', () => {
	...
	it('respects a non-root base path', () => {
		vi.stubEnv('BASE_URL', '/norna/')
		expect(parseTaskIdFromUrl('http://localhost:3000/norna/tasks/7')).toBe(7)
		expect(parseTaskIdFromUrl('/norna/tasks/7')).toBe(7)
		expect(parseTaskIdFromUrl('http://localhost:3000/tasks/7')).toBeNull()
	})

	it('accepts urls from the configured frontend url', () => {
		useConfigStore().frontend_url = 'https://norna.example.com/'

		expect(parseTaskIdFromUrl('https://norna.example.com/tasks/9')).toBe(9)
		...
		expect(parseTaskIdFromUrl('https://norna.example.com.evil.com/tasks/9')).toBeNull()
		expect(parseTaskIdFromUrl('https://evil.norna.example.com/tasks/9')).toBeNull()
	})

	it('respects a path in the configured frontend url', () => {
		useConfigStore().frontend_url = 'https://example.com/norna/'

		expect(parseTaskIdFromUrl('https://example.com/norna/tasks/9')).toBe(9)
	...
	it('only accepts the current origin when no frontend url is configured', () => {
		expect(parseTaskIdFromUrl('http://localhost:3000/tasks/9')).toBe(9)
		expect(parseTaskIdFromUrl('https://norna.example.com/tasks/9')).toBeNull()
	})
